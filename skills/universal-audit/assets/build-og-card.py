...
universal-audit -- 1280x640px GitHub / Twitter OG card, white edition

Layout matches the approved glenski-web-research-mcp card, inverted to a
white field. Run with system Python (Pillow required).
"""
...
INK     = (10, 10, 10)          # heading, was WHITE on dark card
...
# Subtitle
draw.text((PAD, y), "Is this safe to ship? Prove it.", font=f_sub, fill=SUBTLE)
y += SUB_H + GAP_SB
...
# Tool pills
Y_PILLS = y
tools = ["control_catalog", "score.py", "release_gates"]
px = PAD
for label in tools:
...
# Feature tagline
draw.text((PAD, y), "Evidence over assumption.   Deterministic scoring.   Gated verdicts.",
          font=f_feature, fill=FEATURE)
y += FEAT_H + GAP_FD

# Secondary detail
draw.text((PAD, y), "120+ catalogued controls.   Independent challenge.   Machine-readable artifacts.",
...
# Attribution
draw.text((PAD, y), "Evidence-led release assurance", font=f_handle, fill=ATTR)
...
if handle_y + 18 < H - 20:
    draw.text((right_cx - handle_w // 2, handle_y), handle, font=f_handle, fill=ATTR)

# --- Save ------------------------------------------------------------------------------
OUT = r"C:\Users\Glen\Documents\New project\universal-audit-social-preview.png"
canvas.save(OUT, "PNG")
print(f"  Saved: {OUT}")
