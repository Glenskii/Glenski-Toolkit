...
The following declarations pre-configure this skill for Glen E. Grant Creative. They override the skill's baseline defaults where noted. Do not ask Glen to confirm these — they are established.

**Design system (dark-first across all properties):**
- Background: `#0A0A0A` — all Glen sites are dark-first. Light sections are the exception, not the default.
- Primary accent: `#E85D04` (orange)
- Secondary accent: `#E63946` (coral)
- Heading font: **Georgia serif** — this is an APPROVED EXCEPTION to §4.1 serif discipline. Georgia is Glen's established brand voice across every property. Do not replace it with Geist, Cabinet Grotesk, or any sans-display. Do not treat it as a default reach — it is a deliberate brand decision already made.
- Body font: `system-ui, -apple-system, sans-serif` — no Google Fonts on body text.

**Dial presets for Glen's product landing pages (default starting point):**
- `DESIGN_VARIANCE: 7` — confident asymmetry, not chaos
- `MOTION_INTENSITY: 5` — intentional motion, nothing perpetual
- `VISUAL_DENSITY: 4` — product-clear, not sparse

**Typography floor (hard rules, non-negotiable, overrides §4.1 and §6):**
- Body / buttons / inputs / nav: minimum **16px**
- Labels / helper text / captions: minimum **14px** (never text-xs / 12px)
- Headings on mobile: minimum **22px**
- Posters / OG cards / slides: minimum **22px** for any visible text
- Never use Tailwind `text-xs`. Use `text-sm` (14px) or larger.

**Windows font rendering (applies to all Electron and web builds):**
- Never use Tailwind `antialiased` alone on Windows — it is macOS-optimized.
- Always pair: `antialiased subpixel-antialiased` plus `text-rendering: optimizeLegibility` on body-level elements.

**Package manager:** pnpm (default for all Glen projects). Read every `npm install` example in this skill as `pnpm add`. Never yarn.

**Grey text rule (hard stop):**
- No grey text on dark backgrounds. Minimum `#AAAAAA`. Attribution, labels, and secondary text must be white or near-white on `#0A0A0A` fields.

**No em-dash anywhere.** This aligns with the skill's §9.G ban and Glen's CLAUDE.md. Zero tolerance. Use hyphens, commas, colons. No `—` or `–` used as separators.

**Properties in scope for this skill:**
| Property | Type | Notes |
|----------|------|-------|
| watermarkgienie.com | Product landing | Dark, coral accent, desktop app |
| ideathreader.com | SaaS landing | Dark, surgical .htaccess constraint |
| mailmindz.app | SaaS landing | Email intelligence product |
| sitemappro.ca | Tool landing | Cloudflare Worker backend |
| profile.glenegrant.com | Personal brand profile | Primary public-facing identity |
| www.glenegrant.com | Photography business | WordPress + standalone HTML overrides |
| apps.glenegrant.com | App showcases | Subdomains on Cloudflare Pages |
...
* **SERIF DISCIPLINE (VERY DISCOURAGED AS DEFAULT):**
  ...
  * **GLEN EXCEPTION:** Georgia is pre-approved for all Glen E. Grant Creative properties. It is the established brand heading font, not a default reach. When working on any Glen property (see Brand Context above), Georgia headings are correct and do not need justification.
...
- [ ] **Glen typography floor**: all body/inputs/buttons ≥ 16px, labels ≥ 14px, mobile headings ≥ 22px, NO text-xs?
- [ ] **Glen Windows font rendering**: `subpixel-antialiased` + `text-rendering: optimizeLegibility` alongside any Tailwind `antialiased`?
- [ ] **Glen grey-text rule**: no grey text on dark backgrounds, minimum #AAAAAA, attribution and labels white/near-white on #0A0A0A?
...
pnpm add @atlaskit/css-reset @atlaskit/tokens @atlaskit/button @atlaskit/badge @atlaskit/section-message @atlaskit/card
...
A web approximation can use: `backdrop-filter`, transparent backgrounds, layered borders, highlight overlays, gradients, motion, strong contrast fallbacks. But that is **web glassmorphism / frosted-glass approximation**, not official Apple Liquid Glass. Label it as such in comments.
